...
const generateSVG = (skills, theme) => {
    ...
    const skillElements = skills.map((skill, index) => {
        ...
        return `
            <g>
                ...
                <rect x="200" y="${index * 80 + 90}" width="180" height="20" fill="${selectedTheme.progressBarBg}" />
                <rect x="200" y="${index * 80 + 90}" width="${levelPercentage * 1.8}" height="20" fill="${selectedTheme.progressBarFill}" class="progress-bar" />
            </g>
        `;
    }).join('');
    ...
    return `
        <svg width="400" height="${skills.length * 80 + 140}" xmlns="http://www.w3.org/2000/svg">
            ...
            <rect x="0" y="60" width="400" height="10" fill="${selectedTheme.background}"/> <!-- Spacer -->
            ${skillElements}
        </svg>
    `;
};
...
app.get('/api/skills/:username/:theme?', async (req, res) => {
    ...
    try {
        ...
        const skillArray = Object.entries(skillMatrix).map(([skill, { level }]) => ({
            skill,
            level,
        }));
